...
# normalize category list for dropdown
cat_options = []
for c in cats:
...
if not cat_options:
    st.warning("Nu am putut citi categoriile sau lista este goală. Poți crea una nouă.")
...
if st.button("Importă în Gomag", type="primary"):
    ...
    for i, url in enumerate(urls, start=1):
        try:
            out = asyncio.run(process_one_url(url, settings, gomag, st.secrets))
